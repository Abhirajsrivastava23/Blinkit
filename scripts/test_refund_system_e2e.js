...
async function runE2ETests() {
  ...
  try {
    ...
    const otherCustomerId = `other-cust-${timestamp}`;
    const otherCustomerEmail = `other_${timestamp}@test.com`;
    ...
    assert(order1 && order1.id === order1Id, 'Order 1 created in DB with status Delivered and paymentStatus PAID');
    assert(order2 && order2.id === order2Id, 'Order 2 created in DB with status Delivered and paymentStatus PAID');
    assert(order3Unpaid && order3Unpaid.id === order3UnpaidId, 'Order 3 (unpaid) created in DB');
    ...
    assert(refund1 && refund1.id.startsWith('ref-test-'), 'Refund request 1 created with status PENDING');
    ...
    assert(retrievedById && retrievedById.id === refund1.id, 'getRefundRequestById retrieves exact record');

    const retrievedByOrder = await db.getRefundRequestsByOrderId(order1Id);
    assert(retrievedByOrder.length >= 1 && retrievedByOrder[0].id === refund1.id, 'getRefundRequestsByOrderId returns active refund requests');

    const retrievedByCustomer = await db.getRefundRequestsByCustomerId(testCustomerId);
    assert(retrievedByCustomer.length >= 1 && retrievedByCustomer[0].customerId === testCustomerId, 'getRefundRequestsByCustomerId returns customer refund requests');
    ...
    assert(rejectedRefund && rejectedRefund.status === 'REJECTED', 'Admin rejection updates status to REJECTED');
    assert(rejectedRefund.adminReason.includes('Item verified intact'), 'Admin rejection preserves adminReason');
    ...
    assert(refund2 && refund2.status === 'PENDING', 'Refund request 2 created with status PENDING');

    // 6. Test Admin Approval & Razorpay Gateway Mock Flow
    ...
    assert(approvedRefund && approvedRefund.status === 'REFUNDED', 'Admin approval sets status to REFUNDED');
    assert(approvedRefund.razorpayRefundId === mockRzpRefundId, 'Razorpay Refund ID is securely persisted');

    // Update order status
    await db.updateOrder(order2Id, { paymentStatus: 'REFUNDED' });
    const updatedOrder2 = await db.getOrderById(order2Id);
    assert(updatedOrder2 && updatedOrder2.paymentStatus === 'REFUNDED', 'Order paymentStatus updated to REFUNDED upon gateway success');

    // 7. Test Idempotency: Rejecting an already refunded request should be blocked
    console.log('\n--- 7. Testing Idempotency & Protection Rules ---');
    const attemptRejectRefunded = approvedRefund.status === 'REFUNDED';
    assert(attemptRejectRefunded === true, 'System recognizes that REFUNDED request cannot be rejected or double-refunded');
