...
// The curated unique Unsplash images map
const imageMap = {
  // Cakes (20)
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (currentId && imageMap[currentId]) {
    ...
    if (imageMatch) {
      lines[i] = line.replace(/(image:\s*')[^']+'/, `$1${imageMap[currentId]}'`);
      replacedCount++;
      currentId = null; // Reset after replace
    }
  }
  
  // Reset ID when block ends
  if (line.trim() === '},') {
    currentId = null;
  }
}
...
console.log(`Successfully replaced ${replacedCount} product images in mockData.ts.`);
