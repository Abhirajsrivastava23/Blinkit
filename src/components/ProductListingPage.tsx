        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              {/* Toolbar */}
              <div className="bg-white border border-zinc-200/20 rounded-2xl p-2.5 sm:p-4 flex items-center justify-between shadow-sm gap-2 flex-wrap sm:flex-nowrap">
                
                {/* Mobile Filter Button */}
                <button
                  onClick={() => setShowMobileFilters(true)}
                  className="lg:hidden flex items-center gap-1.5 text-[9px] uppercase tracking-widest font-extrabold text-brand-burgundy border border-zinc-200 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-white shrink-0"
                >
                ...
                {/* Counter */}
                <span className="text-[11px] sm:text-xs text-zinc-500 font-medium shrink-0 hidden xs:inline">
                  Showing <strong>{filteredProducts.length}</strong> items
                </span>

                {/* Sorter */}
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                  <span className="text-[8px] sm:text-[9px] font-extrabold uppercase tracking-widest text-zinc-400">Sort</span>
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value)}
                    className="text-[11px] sm:text-xs bg-zinc-50 border border-zinc-250 rounded-xl p-1.5 sm:p-2 outline-none font-semibold text-zinc-700"
                  >
